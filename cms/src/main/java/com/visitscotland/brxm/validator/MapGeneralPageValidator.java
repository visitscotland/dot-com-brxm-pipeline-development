...
/**
 * jcr:nane = visitscotland:map-general-page-validator
 */
public class MapGeneralPageValidator implements Validator<Node> {

    private static final String CMS_LINK_TYPE = "visitscotland:CMSLink";
    private static final Logger logger = LoggerFactory.getLogger(MapGeneralPageValidator.class);

    @Override
    public Optional<Violation> validate(ValidationContext validationContext, Node node) {
        try {
            NodeIterator itemIterator = node.getNodes("hippotaxonomy:keys");
            Node firstNode = itemIterator.nextNode();
            if (itemIterator.getSize() > 1) {
